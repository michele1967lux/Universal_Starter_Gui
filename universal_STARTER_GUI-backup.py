...
class App(ctk.CTk):
    ...
    def setup_gui(self):
        ...
        # Files section
        files_label = ctk.CTkLabel(self, text="File da Avviare:", font=("Arial", 14, "bold"))
        files_label.pack(pady=(10, 5), padx=10, anchor="w")
        
        # Scrollable frame for files
        self.files_frame = ctk.CTkScrollableFrame(self, height=300)
        self.files_frame.pack(pady=5, padx=10, fill="both", expand=True)
        
        # Add file button
        add_file_btn = ctk.CTkButton(self, text="➕ Aggiungi File", command=self.add_file)
        add_file_btn.pack(pady=5)

        # Shell launch option
        self.shell_checkbox = ctk.CTkCheckBox(self, text="Lancia in nuova shell")
        ...
        # Log output section
        log_label = ctk.CTkLabel(self, text="Log Output:", font=("Arial", 14, "bold"))
        log_label.pack(pady=(10, 5), padx=10, anchor="w")

        self.log_console = ctk.CTkTextbox(self, height=150, state="disabled")
