...
class GitOperationManager:
        ...
        def worker():
            try:
                ...
                if not self.cancel_requested:
                    # Callback per completare l'operazione nella GUI
    ...
    def cancel_operation(self, operation_name: str):
        """Richiesta cancellazione operazione."""
        if operation_name in self.active_operations:
            self.cancel_requested = True
            self.ui_callback.on_git_operation_cancelled(operation_name)
...
"""
    ...
    def on_git_operation_completed(self, operation_name: str, result):
        ...
        if success:
            ...
            self.after(500, self.refresh_git_status)
        else:
    ...
    def on_git_operation_cancelled(self, operation_name: str):
        """Callback chiamato quando un'operazione Git è cancellata."""
        self.git_progress_bar.set(0)
        self.git_status_label.configure(text=f"⚠ {operation_name} annullato", text_color="orange")
        log_queue.put(f"[GIT] Operazione '{operation_name}' annullata dall'utente\n")

        self.after(1000, lambda: self._reset_git_ui_state())

    def cancel_git_operation(self):
        """Annulla l'operazione Git corrente."""
        # Trova l'operazione attiva
        active_ops = [op for op in ["checkout", "commit", "push", "merge", "revert", "resume", "stage", "unstage"] if self.git_op_manager.is_operation_active(op)]
        if active_ops:
            self.git_op_manager.cancel_operation(active_ops[0])
